    ...
    public static OperationRoot getOperations(SeekIterator<Token> tokens) {
        ...
        while(tokens.hasNext()) {
            ...
            if(!(token instanceof NamedToken)) {
                throw new TokenLocatedException("Expected NamedToken but got " + token.getClass().getSimpleName(), token);
            }
            NamedToken named = token.asNamedToken();
            switch (named.name) {
                case "Operator" -> {
                    String operatorName = named.tokens[0].asWordToken().getWord();
                    LanguageOperator operator = LanguageOperator.OPERATORS.get(operatorName);
                    if (operator == null) {
                        throw new TokenLocatedException("Unknown operator '" + operatorName + "'", named);
                    }
            ...
            while (cachedOperator.size() != 0) {
                if (tokens.hasNext()) {
                    ...
                    if(!t.name.equals("Operator")) {
                        throw new TokenLocatedException("Expected Operator but got " + t.name, t);
                    }
                    String operatorName = t.tokens[0].asWordToken().getWord();
                    LanguageOperator operator = LanguageOperator.OPERATORS.get(operatorName);
                    if (operator == null) {
                        throw new TokenLocatedException("Unknown operator '" + operatorName + "'", t);
                    }
                    int a = operator.precedence();
                    int b = cachedOperator.peek().precedence();
                    if (a > b) {
